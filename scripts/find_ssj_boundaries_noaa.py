...
from ssj_auroral_boundary.absatday import absatday_poes

from ssj_auroral_boundary import loggername_poes
log = logging.getLogger(loggername_poes)
...
if __name__=='__main__':
    ...
    parser = argparse.ArgumentParser(description="POES/MetOp TED auroral boundary identification")

    parser.add_argument("poes_number",
                            type=int,
                            help='Process TED for POES/MetOp-## where this argument is ##',
                            default=None)
    ...
    if args.test:
        ...
        #Use the test NC file, overriding the satnum and date settings
        test_data_dir,test_ncfn = files.test_nc_path_and_filename()
        ncfn = os.path.join(test_data_dir,test_ncfn)
        shutil.copy(ncfn,dirs['data'])

        log.warning('--test command line argument overrides '
                    'satellite, year, month, day settings')
    else:

        #Try to download NC if can't find it in the data directory
        url_and_fn = files.nc_url_and_filename(poes_number,year,month,day)
        nc_url,ncfn = url_and_fn
        ncfn = os.path.join(dirs['data'],ncfn)
        if not os.path.exists(ncfn):
            log.info('NC %s does not exist, attempting to download' % (ncfn))
            files.download_nc_from_noaa(nc_url,ncfn)

    log.info('Beginning run of NC file %s' % (ncfn))

    absd = absatday_poes(ncfn,
                         imgdir=dirs['image'],
                         csvdir=dirs['csv'],
